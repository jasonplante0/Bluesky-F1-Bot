...
export default class Bot 
{
  ...
  */
    ...
    var bskyFeedAwait = await this.userAgent.app.bsky.feed.getAuthorFeed({actor: "notf1.bsky.social", limit: postNum,}); // Get a defined number + 2 of most recent posts from the logged in user's feed.
  ...
  ) 
  {
    ...
    if (urls[0].slice(-3) == "mp4" && parseFloat(alts[0].split("@#*")[2]) < 180
      ...
      if (card != "None" && urls[0] == "None")
      {
        ...
        var cardBuffer = Buffer.from(cardResponse.data, "utf-8");
          if (cardBuffer.length > 1000000)
          {
            ...
            cardResponse = await axios.get("https://pbs.twimg.com/profile_images/1612433922733887489/7f5XFklA_400x400.jpg", { responseType: 'arraybuffer'}); 
